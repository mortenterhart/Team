...
public class OrderedCrossover implements ICrossover {
    public Tour doCrossover(Tour tour01,Tour tour02) {
        ...
        int rand1 = Configuration.instance.mersenneTwister.nextInt(0, size - 1);
        int rand2 = Configuration.instance.mersenneTwister.nextInt(rand1, size - 2);
        ...
        int toSaveIdx1 = 0, toSaveIdx2 = 0;
        //loop through all parent elements
        for (int i = 0; i < size; i++) {
            if(toSaveIdx1 == start) { //check if we need to skip already copied slice
                toSaveIdx1 = end;
            }
            if(toSaveIdx2 == start) {
                toSaveIdx2 = end;
            ...
            if(!child1.containsCity(c2)) {
                child1.addCity(toSaveIdx1, c2);
                toSaveIdx1++;
            }
            if(!child2.containsCity(c1)) {
                child2.addCity(toSaveIdx2, c1);
                toSaveIdx2++;
            }
